...
int level = 0;  //the smaller the quicker
int bypassCounter = 0;
int hitCounter = 0;  //nb hit
bool ping = true;

const byte digit[10][WSCORE * HSCORE] = {
  // digit de 0 à 9 sur 2 cellules par 4
...
void loopPong()
{
  ...
  while (!fin) {
    ...
    if (minitel.getKeyCode() == CONNEXION_FIN) {
...
void playGame() {
 handlePlayer();
  ...
  if (startingCounter > 0) {
    ...
    //updateLevel
    if (hitCounter == 3) level = 999;
    if (hitCounter == 6) level = 999;
    // if (bypassCounter < level) { //viré par Cécile
    if (1 < 0) {
